...
public class TweetUtils {
    ...
    public static String stripPunct(String word) {
        ...
        // strip leading
        result = word.replaceFirst("^[^a-zA-Z]+", "");
        // strip trailing
     ...
     * <br />
     * - KEY: STRING word1<br />
     * - VALUE: LIST { nextword1, nextword2, ... , nextwordN }<br />
     * <br />
    ...
    public static HashMap<String, List<String>> markov(List<String> statuses) {
        ...
        for (int s = 0; s < statuses.size(); s++) {
            ...
            for (int w = 0; w < words.length - 1; w++) {
                ...
                // get the word and the next word
                String word = words[w];
                String nextWord = words[w + 1];

                // if it's not acceptable, then fuhgeddaboudit
                if (!isAcceptable(word) || !isAcceptable(nextWord))
                    continue;

                // if either word is empty after punct is stripped
                if (stripPunct(word).equals("")
                        || stripPunct(nextWord).equals(""))
                    continue; // then forget it

                // get the following words, or new list if null
                List<String> followingWords = result.get(word);
                if (followingWords == null)
                    followingWords = new ArrayList<>();

                // add to the list and put it back
                followingWords.add(nextWord);
                result.put(word, followingWords);
